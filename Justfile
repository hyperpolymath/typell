...
init:
    ...
    # Also replace typell and hyperpolymath in AI manifest
...
cookbook:
    ...
    echo "= typell Justfile Cookbook" > "$OUTPUT"
...
man:
    #!/usr/bin/env bash
    mkdir -p docs/man
    cat > docs/man/typell.1 << EOF
    .TH typell 1 "$(date +%Y-%m-%d)" "{{version}}" "typell Manual"
    .SH NAME
    typell \- RSR-compliant project
    .SH SYNOPSIS
    .B just
    [recipe] [args...]
    .SH DESCRIPTION
    ...
    EOF
    echo "Generated: docs/man/typell.1"
...
container-init:
    ...
    read -rp "Service name (e.g. my-api) [typell]: " _SERVICE_NAME
    SERVICE_NAME="${_SERVICE_NAME:-typell}"
...
container-build *args:
    ...
    if [ -f "container/ct-build.sh" ]; then
        ...
        podman build -t typell:latest -f container/Containerfile .
    elif [ -f "Containerfile" ]; then
        podman build -t typell:latest -f Containerfile .
    else
...
container-push:
    ...
    if [ -f "container/ct-build.sh" ]; then
        ...
        podman push typell:latest
    fi

# Run container interactively (for debugging)
container-run *args:
    podman run --rm -it typell:latest {{args}}
...
doctor:
    @echo "Running diagnostics for typell..."
...
heal:
    @echo "Attempting auto-repair for typell..."
...
help-me:
    ...
    @echo "  https://github.com/hyperpolymath/typell/issues/new"
    @echo ""
