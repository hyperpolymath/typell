...
│       └── include/
│           └── typell.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── typell.h       # Generated from Idris2 ABI
│
...
int main() {
    void* handle = typell_init();
    if (!handle) return 1;

    int result = typell_process(handle, 42);
    if (result != 0) {
        const char* err = typell_last_error();
...
extern "C" {
    ...
    fn typell_free(handle: *mut std::ffi::c_void);
    fn typell_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = typell_init();
        assert!(!handle.is_null());

        let result = typell_process(handle, 42);
        assert_eq!(result, 0);
...
function init()
    handle = ccall((:typell_init, libtypell), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:typell_process, libtypell), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:typell_free, libtypell), Cvoid, (Ptr{Cvoid},), handle)
end
