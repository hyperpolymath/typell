...
const VERSION = "0.1.0";
const BUILD_INFO = "TYPELL built with Zig " ++ @import("builtin").zig_version_string;
...
export fn typell_free(handle: ?*Handle) void {
...
export fn typell_process(handle: ?*Handle, input: u32) Result {
...
export fn typell_get_string(handle: ?*Handle) ?[*:0]const u8 {
    const h = handle orelse {
        setError("Null handle");
        return null;
    };
...
export fn typell_free_string(str: ?[*:0]const u8) void {
...
export fn typell_process_array(
    handle: ?*Handle,
    buffer: ?[*]const u8,
    len: u32,
) Result {
...
export fn typell_last_error() ?[*:0]const u8 {
...
export fn typell_version() [*:0]const u8 {
    return VERSION.ptr;
}

/// Get build information
export fn typell_build_info() [*:0]const u8 {
...
/// Register a callback
export fn typell_register_callback(
    handle: ?*Handle,
    callback: ?Callback,
) Result {
    const h = handle orelse {
...
export fn typell_is_initialized(handle: ?*Handle) u32 {
...
test "lifecycle" {
    ...
    try std.testing.expect(typell_is_initialized(handle) == 1);
}

test "error handling" {
    const result = typell_process(null, 0);
